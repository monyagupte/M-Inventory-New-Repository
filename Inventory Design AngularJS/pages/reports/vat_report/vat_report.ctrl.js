var scotchApp = angular.module('scotchApp.vatReport', ['ngRoute']);
scotchApp.controller('vatReportController', function ($rootScope, $scope, $http, $location, $routeParams, $filter) {
    ...
    $scope.ShowRecords = function () {
        ...
        // alert(startDate);
        // alert(endDate);
        //http://minvent.azurewebsites.net/api/Sp_vat?startdate=2015-12-01&enddate=2016-1-31
        $http.get('http://minvent.azurewebsites.net/api/Sp_vat?startdate=' + startDate + '&enddate=' + endDate)
             .success(function (res) {
                 ...
                 for (var i = 0; i <= count; i++) {
                     if (data[i].PurchaseTaxRate=='Total') {
                         
                         $scope.purchaseTaxTotal = data[i].InputTaxAmount;
                         $scope.saleTaxTotal = data[i].OutputTaxAmount;

                         $scope.amountPayable = $scope.saleTaxTotal - $scope.purchaseTaxTotal;
